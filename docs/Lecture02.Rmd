...
> 講義的[源始Rmd](Lecture02.Rmd);所有課程學員請下載[作業Rmd](Lecture02HW.Rmd),種子教師工作坊請下載[增能指引](Lecture02after.md)。
...
|:---:|:---:|:---:|
|We claim no effect<br>(ES=0)|Correct conclusion<br>($p = 1 - \alpha$)|Type II error<br>($p = \beta$)|
|We claim an effect<br>(ES $\neq$ 0)|Type I error<br>($p = \alpha$)|Correct conclusion<br>($p = 1 - \beta$)|
||Universe X|Universe Y|

- 為研究結果負責的研究者，應該有能力判斷最合適的$\frac{\alpha}{\beta}$s
...
- 各種研究場域都有合適的估算工具，研究者應以**現實需要**選擇工具。
...
## two tailed test
pwr::pwr.t.test(d=unlist(effectsize::t_to_d(1.61, 11))["d"],
                power = .80,
                type = "two.sample",
...
pwr::pwr.t.test(n=6,
                d=unlist(effectsize::t_to_d(1.61, 11))["d"],
                type = "two.sample",
                alternative = "two.sided")
```
...
種子教師工作坊請下載請下載[增能指引](Lecture02after.md)，延續學習成果。
