...
all_data <- bind_rows(
  ...
  pivot_longer(NT001:NT024,names_to = "ID",values_to = "Response") %>% bind_cols(Group = "Normal",Condition = "Face"),

readxl::read_xlsx("curve fitting data.xlsx",sheet = "Autism_FACE") %>%
  pivot_longer(AS001:AS024,names_to = "ID",values_to = "Response") %>% bind_cols(Group = "Autisum",Condition = "Face"),

readxl::read_xlsx("curve fitting data.xlsx",sheet = "Normal_OBJ") %>%
  pivot_longer(NT001:NT024,names_to = "ID",values_to = "Response") %>% bind_cols(Group = "Normal",Condition = "Object"),

readxl::read_xlsx("curve fitting data.xlsx",sheet = "Autism_OBJ") %>%
  pivot_longer(AS001:AS024,names_to = "ID",values_to = "Response") %>% bind_cols(Group = "Autisum",Condition = "Object"))

```

@xiongenyunBiJiaoZiBiZhengChengRenYuYiBanChengRenDeLianKongHeWuTiQuBianNengLiYiBianXingDianFanTanTao2022 比較24位自閉症成年人士及24位心智正常人士，進行變形臉孔區辨作業與變形物體區辨作業，比較自閉症族群與正常人士辨識臉孔與物體正確率達75%的最低變形程度閾值。變形臉孔區辨作業操作臉孔變形程度(15%,30%,45%,60%,75%)，變形物體區辨作業操作臉孔變形程度(12%,24%,36%,48%,60%)，研究者紀錄參與者能正確辨識目標臉孔的次數。研究結果顯示只有變形臉孔區辨作業的部分條件 出現組間差異，若要重製或延伸這項研究，我們能如何評估效果量及事後考驗力？

感謝簡惠玲老師提供[原始研究資料](files/curve fitting data.xlsx)。歡迎使用[本文源始Rmd](files/example.Rmd)及[參考文獻bib](files/examples.bib)重製及編輯分析結果。

## 重製原始論文圖4

原始資料格式是寬表單，轉換為長表單以利重製。首先計算個別參與者在不同變形程度的正確率(Acc)，再依作業情境(Condition: Face, Object)、參與者組別(Group: Autism, Normal)以及變形程度(參考前一節)總計各組正確次數總和(SUM), 平均正確率(Mean)、正確率估計標準誤。
...
此外 @xiongenyunBiJiaoZiBiZhengChengRenYuYiBanChengRenDeLianKongHeWuTiQuBianNengLiYiBianXingDianFanTanTao2022 的報告對於信賴區間的解釋並不正確，寫作亦不符合APA格式。
...
一般成人組區辨閾值為`r round(100*subset(fitFace$thresholds,Group=="Normal")$thre,2)`% 95% CI [`r round(100*subset(fitFace$thresholds,Group=="Normal")$threinf,2)`, `r round(100*subset(fitFace$thresholds,Group=="Normal")$thresup,2)`]；ASD組的區辨閾值為`r round(100*subset(fitFace$thresholds,Group=="Autisum")$thre,2)`% 95% CI[`r round(100*subset(fitFace$thresholds,Group=="Autisum")$threinf,2)`, `r round(100*subset(fitFace$thresholds,Group=="Autisum")$thresup,2)`]。兩組區辨閾值差異估計值為`r round(100*fitFace$thresholdcomparisons["dif"],2)`% 95% CI[`r round(100*fitFace$thresholdcomparisons["difinf"],2)`, `r round(100*fitFace$thresholdcomparisons["difsup"])`]。
...
一般成人組區辨閾值為`r round(100*subset(fitFace$thresholds,Group=="Normal")$thre,2)`% 95% CI [`r round(100*subset(fitObj$thresholds,Group=="Normal")$threinf,2)`, `r round(100*subset(fitObj$thresholds,Group=="Normal")$thresup,2)`]；ASD組的區辨閾值為`r round(100*subset(fitObj$thresholds,Group=="Autisum")$thre,2)`% 95% CI[`r round(100*subset(fitFace$thresholds,Group=="Autisum")$threinf,2)`, `r round(100*subset(fitObj$thresholds,Group=="Autisum")$thresup,2)`]。兩組區辨閾值差異估計值為`r round(100*fitObj$thresholdcomparisons["dif"],2)`% 95% CI[`r round(100*fitObj$thresholdcomparisons["difinf"],2)`, `r round(100*fitObj$thresholdcomparisons["difsup"])`]。
