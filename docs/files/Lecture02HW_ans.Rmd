...
```{r}
result1_d <- round(unlist(effectsize::t_to_d(2.35, 23,paired = TRUE))["d"],2)
effectsize::interpret_cohens_d(result1_d)
```
...
```{r}
result2_d <- round(unlist(effectsize::t_to_d(3.2, 34))["d"],2)
effectsize::interpret_cohens_d(result2_d)
```
...
```{r}
result3_d <- round(unlist(effectsize::t_to_d(2.8, 40,paired = TRUE))["d"],2)
effectsize::interpret_cohens_d(result2_d)
```
...
```{r}
pwr.t.test(d=.27,power = .9, sig.level = .01,type = "one.sample",alternative = "two.sided")
```
