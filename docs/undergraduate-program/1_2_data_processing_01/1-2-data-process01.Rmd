...
學習重點有常用的六種`dplyr`(整合於`tidyverse`)資料整頓函式：`select()`,`filter()`,`mutate()`,`arrange()`,`group_by()`,`summarise()`，以及柱狀圖/箱形圖為範例的ggplot2視覺化語法。
...
- 參考Level 1單元8，完成能回答以下問題的散佈圖

```{r positive-data-scatterplot01}
## 編輯程式碼，重製Figure 8.1
```
...
```{r positive-data-scatterplot02}
## 編輯程式碼，重製Figure 8.3
```

> 根據上圖，能否判斷兩種量表評分有性別差異？


```{r positive-data-scatterplot02}
## 編輯程式碼，以幸福感評分74分區別兩組受測者，重製Figure 8.4
```
